...
class FbrefPlayersSpider(scrapy.Spider):
    name = 'fbref_players_spider'
    ...
    def parse(self, response, *args, **kwargs):
        ...
        for row in rows_list:
            ...
            for col in range(1, columns_len):
                if col == 1:
                    element = driver.find_element("xpath", f'//*[@id="stats_standard"]/tbody/tr[' + str(row) + ']/th[1]').text
                    each_row.append(element)
                    element = driver.find_element("xpath", f'//*[@id="stats_standard"]/tbody/tr[' + str(row) + ']/td[' + str(col) + ']').text
                    each_row.append(element)
                else:
                    element = driver.find_element("xpath", f'//*[@id="stats_standard"]/tbody/tr[' + str(row) + ']/td[' + str(col) + ']').text
                    each_row.append(element)
        ...
        with open('/opt/airflow/data/fbref_players.csv', 'w') as file:
            file.write('')
        pd.to_csv("/opt/airflow/data/fbref_players.csv", index=False, header=False)
